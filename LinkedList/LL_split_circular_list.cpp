...
void print(struct node *node)
{
	...
	cout<<node->data<<endl;
	cout<<"end of unique nodes"<<endl;
}

void main_list_circular(struct node *head)
{
	struct node *temp=head;
	while(temp->next!=NULL)
	temp=temp->next;
...
struct node* make_it_circular(struct node **head)
{
	struct node *head2=NULL;
	int data=(*head)->data;
	struct node *ptr1=(*head)->next->next, *ptr2=*head;
	cout<<"ptr1->data"<<ptr1->data;
	while(ptr1->next->data!=data)
	{
		if(ptr1->next->next->data!=data)
		{
			ptr1=ptr1->next->next;
			ptr2=ptr2->next;
		}
...
int main()
{
 ...
 // cout<<"Control returned to main";
  struct node *head2=NULL;
  cout<<"Calling function"<<endl;
  main_list_circular(head);
    print(head);
  head2=make_it_circular(&head);
  cout<<"List is now circular"<<endl;
